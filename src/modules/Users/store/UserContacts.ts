...
export const useUserContactsStore = defineStore('userContacts', {
  ...
  getters:{
    ...
    getActiveContact(state) {
      if(state.activeContact != null) {
        return state.contacts.find(u => u.contactId === state.activeContact)
      }
      return null
    },
  ...
  actions:{
    ...
    setChatMessageIsRead: function(message: IMessage) {
      const contact = this.contacts.find(c => c.chatId === message.chatId)
      const result = contact?.chat.chatMessages.find(m => m.id === message.id)
    ...
    addMessageToChat: function(message: IMessage) {
      const contact = this.contacts.find(c => c.chatId === message.chatId)

      if(contact) {
        contact.chat.chatMessages.push(message)
       }
    },

    setActiveContactChat: function(contactId: string) {
    ...
    addContact: function(contact: IContact) {
