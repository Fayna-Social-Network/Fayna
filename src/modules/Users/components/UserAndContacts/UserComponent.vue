<template>
  <Loader v-if="loading" />
  <Contact @click="userClickHandle" v-else>
    <template #profile>
      <div class="profile">
    ...
    <template #content>
      <div class="action-buttons">
        <q-btn v-if="checkBlockUsers" round color="accent" size="12px" class="glossy" icon="block"
        ...
        <q-btn v-else-if="blockMe" round color="negative" size="12px" class="glossy" icon="remove_circle_outline">
          <q-tooltip anchor="center right" self="center left" :offset="[10, 10]" transition-show="flip-right"
            transition-hide="flip-left">
            {{ $t('YouAreBlocked') }}
          </q-tooltip>
        </q-btn>
        <q-btn v-else-if="userContact == undefined" round color="primary" size="12px" class="glossy" icon="person_add_alt"
        ...
        <q-btn v-else round color="negative" class="glossy" size="12px" icon="person_remove"
