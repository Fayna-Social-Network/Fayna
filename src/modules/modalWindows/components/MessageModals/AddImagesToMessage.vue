  ...
  <DialogModal>
    ...
    <template #body>
      ...
      <div class="content" v-else>
        <q-uploader :label="$t('UploadImages')" multiple hide-upload-btn :filter="checkFileType"
          :no-thumbnails="$q.platform.is.mobile" @added="(addedImagesHandle as any)"
          @removed="(removedImagesHandle as any)" @rejected="onRejected" style="max-width: 300px" />
        <div class="album-addition">
          ...
          <div class="inputs">
            <transition-group name="list" tag="p">
              ...
              <q-input v-model="description" type="textarea" :label="$t('AddAlbumDesc')" v-if="addDesc" />
            </transition-group>
          </div>
        </div>
      </div>
    ...
    <template #actions>
      <q-btn color="primary" :label="$t('Send')" :disable="files.length === 0 || inprocces" @click="SendImageHandle()" />
      <q-btn color="negative" :label="$t('Cancel')" :disable="inprocces" @click="closeHandle()" />
    </template>
  </DialogModal>
</template>
...
import ImageService from "src/modules/Message/services/messages/imageMessage.service"
