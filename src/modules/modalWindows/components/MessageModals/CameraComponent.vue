      ...
      <div class="camera-body">
        ...
        <div class="screens">
          ...
          <div class="source-controls">
            <q-btn icon="videocam" @click="toggleSource('camera')" :color="source === 'camera' ? 'accent' : 'secondary'"
              :class="{ 'active': source === 'camera' }" />
            <q-btn icon="desktop_windows" @click="toggleSource('screen')"
              :color="source === 'screen' ? 'accent' : 'secondary'" :class="{ 'active': source === 'screen' }" />
          </div>
        ...
        <div class="controls">
          <div class="group">
            <q-btn :color="startRecordingButtonText === 'radio_button_checked' ? 'negative' : 'primary'" glossy
              :icon="startRecordingButtonText" @click="recordHandle" :disabled="recordDisabled">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left">
                {{ $t('Record') }}
              </q-tooltip>
            </q-btn>

            <q-btn color="primary" glossy icon="play_arrow" @click="playHandle" :disabled="playDisabled">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left">
                {{ $t('Play') }}
              </q-tooltip>
            </q-btn>

            <q-btn color="primary" glossy icon="screenshot" @click="getScreenShotHandle" :disabled="snapDisabled">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left">
                {{ $t('Screenshot') }}
              </q-tooltip>
            </q-btn>

            <q-btn color="primary" glossy icon="file_download" @click="downloadHandle" :disabled="dowloadDisabled">
              <q-tooltip transition-show="flip-right" transition-hide="flip-left">
                {{ $t('Download') }}
              </q-tooltip>
            </q-btn>
          ...
          <div class="group">
            <div class="record-format">
              {{ $t('RecordingFormat') }}
              <select v-model="codecPreferences" ref="codecSelect" :disabled="codecDisabled"></select>
            </div>
          </div>
        </div>

        <div class="details">
          <q-toggle v-model="hasEchoCancellation" checked-icon="check" color="green" unchecked-icon="clear"
            :label="$t('EchoCancellation')" />
        </div>
      </div>
    </template>
    <template #actions>
      <q-btn color="primary" round icon="send" :disabled="!getScreenshot && playDisabled" @click="sendMessageHandler" />
    </template>
  </DialogModal>
</template>
...
import ImageMessage from "src/modules/Message/services/messages/imageMessage.service"
import { IContact } from "src/modules/Users/types/contact"
import { mapState, mapActions } from "pinia"
import { useUserStore } from "src/modules/Users/store/User"
import { useMainStore } from "stores/Main"
import VideoMessage from "src/modules/Message/services/messages/videoMessage.service";
